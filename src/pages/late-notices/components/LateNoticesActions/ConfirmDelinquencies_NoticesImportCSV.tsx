...
import { convertDateStringToUTCISOString, formatCurrency, formatZip, handlePostalCodeKeyDown } from "utils/helper";
import { ConfirmDelinquenciesNoticesCSVHeader, StateCode } from "utils/constants";
import CommonValidations from "utils/common-validations";
import {
    IImportCsvFieldError,
    IImportCsvRowError,
} from "interfaces/common.interface";
import { IConfirmDelinquenciesImportItems, ISelectOptions } from "interfaces/late-notices.interface";
...
type ConfirmDelinquenciesNoticesImportCsvProps = {
...
const ConfirmDelinquenciesNotices_ImportCsv = (props: ConfirmDelinquenciesNoticesImportCsvProps) => {
    const {
        allCompanies,
    ...
    const validationSchema: yup.ObjectSchema<any> = yup.object({
        ...
        // OwnerName: yup.string().max(50, "Owner name must not exceed 50 characters"),
        PropertyName: yup
            .string()
            // .max(50, "Property must not exceed 50 characters")
   ...
   .test('is-valid-phone', 'Please enter a valid phone number', value => {
      ...
      return /^(\(\d{3}\) ?|\d{3}-?)\d{3}-?\d{4}$/.test(value);
        ...
        NoticeServerID:yup.string().required("Server is required."),
    });
    ...
    const [gridData, setGridData] = useState<IConfirmDelinquenciesImportItems[]>([]);
    ...
    const [filteredRecords, setFilteredRecords] = useState<IConfirmDelinquenciesImportItems[]>(
        []
    );
    ...
    const handleEvictionsNotices = async () => {
        ...
        if (errors.length === 0) {
            try {
                ...
                const formattedData = gridData.map((item: IConfirmDelinquenciesImportItems) => ({
                    ...item,
                    MonthlyRent: item.MonthlyRent ? parseFloat(
                        item.MonthlyRent.toString()
                            .replace(/[^0-9.]/g, "")
                            .replace("$", "")
                    ) : 0,
                    NoticePastRentDue: item.NoticePastRentDue ? parseFloat(item.NoticePastRentDue.toString().replace(/[^0-9.]/g, "")) : 0,
                    NoticeCurrentRentDue: item.NoticeCurrentRentDue ? parseFloat(item.NoticeCurrentRentDue.toString().replace(/[^0-9.]/g, "")) : 0,
                    NoticeLastPaidAmount: item.NoticeLastPaidAmount ? parseFloat(item.NoticeLastPaidAmount.toString().replace(/[^0-9.]/g, "")) : 0,
                  //  NoticeTotalDue: item.NoticeTotalDue ? parseFloat(item.NoticeTotalDue.toString().replace(/[^0-9.]/g, "")) : 0,
                    NoticeLateFees: item.NoticeLateFees ? parseFloat(item.NoticeLateFees.toString().replace(/[^0-9.]/g, "")) : 0,
                    // AndAllOtherOccupants: item.andAllOtherTenants ? item.andAllOtherTenants : "",
                    PropertyPhone: item.PropertyPhone ? item.PropertyPhone.replace(/[^\d-]/g, "").replace(/(\d{3})(\d{3})(\d{4})/, "$1-$2-$3") : "",
                    // EvictionTotalRentDue: parseFloat(
                    //   item.EvictionTotalRentDue.toString().replace(/[^0-9.]/g, "").replace('$', '')
                    // ).toString(),
                    AttorneyBarNo: item.AttorneyBarNo ? item.AttorneyBarNo.toString():"",
                    ClientReferenceId: item.ClientReferenceId ? item.ClientReferenceId.toString():"",
                    NoticeServerID: item.NoticeServerID ? item.NoticeServerID.toString():"",
                    TenantZip: item.TenantZip != null ? formatZip(item.TenantZip) : "",
                     PropertyZip: item.PropertyZip != null ? formatZip(item.PropertyZip) : "",
                    NoticeDefaultEndDate: item.NoticeDefaultEndDate ? convertDateStringToUTCISOString(item.NoticeDefaultEndDate as string) : undefined,
                    NoticeDeliveryDate: item.NoticeDeliveryDate ? convertDateStringToUTCISOString(item.NoticeDeliveryDate as string) : undefined,
                    NoticeLastPaidDate: item.NoticeLastPaidDate ? convertDateStringToUTCISOString(item.NoticeLastPaidDate as string) : undefined,
                    NoticeDefaultStartDate: item.NoticeDefaultStartDate ? convertDateStringToUTCISOString(item.NoticeDefaultStartDate as string) : undefined,
                    NoticeLateFeeDate:item.NoticeLateFeeDate ? convertDateStringToUTCISOString(item.NoticeLateFeeDate as string) : undefined,
                }));
                if (userRole.includes(UserRole.C2CAdmin)||userRole.includes(UserRole.ChiefAdmin)) {
                    if (selectedCompany.id != '' && selectedCompany.value != '') {
                        ...
                        const response = await LateNoticesService.createEvictionNotice(
                            formattedData
                        );
                        if (response.status === HttpStatusCode.OK) {
                            toast.success("Successfully Imported CSV");
                            getConfirmDelinquenciesQueue(1, 100, false, false);
    ...
    const validateRow = (row: IConfirmDelinquenciesImportItems, rowIndex: number) => {
        const recordErrors: Record<
            string,
            { rowIndex: number; errorMessage: string }[]
        > = {};
     ...
     */
    const loadUserData = (data: IConfirmDelinquenciesImportItems[]) => {
        try {
            ;
            if (data.length === 0) {
            ...
            const formattedData = data.map((item: IConfirmDelinquenciesImportItems) => {
                return {
                    Remove: "",
                    ...item,
                    ...
                    //     : "",
                    TenantZip: item.TenantZip
            ? item.TenantZip.toString().slice(0, 5) // Take only the first 5 characters
            : "",
                    PropertyZip: item.PropertyZip
                        ? item.PropertyZip.toString().slice(0, 5)
                        : "",
                   ...
                   // NoticeTotalDue: item.NoticeTotalDue
                     //   ? parseFloat(item.NoticeTotalDue.toString().replace(/[^0-9.]/g, ""))
                       // : 0,
                       NoticeLateFees: item.NoticeLateFees
                        ? parseFloat(item.NoticeLateFees.toString().replace(/[^0-9.]/g, ""))
                        : 0,
                    // BarNo:parseFloat(
                    ...
                    PropertyPhone: item.PropertyPhone ? item.PropertyPhone.toString().trim() : "",
                    // PropertyPhone: item.propertyPhone.toString().trim(),
                    FilerPhone: item.FilerPhone ? item.FilerPhone.toString().trim() : "",
                    NoticeDefaultEndDate: formatDate(item.NoticeDefaultEndDate as string) ?? "",
                    NoticeDeliveryDate: formatDate(item.NoticeDeliveryDate as string) ?? "",
                    NoticeLastPaidDate: formatDate(item.NoticeLastPaidDate as string) ?? "",
                    NoticeDefaultStartDate: formatDate(item.NoticeDefaultStartDate as string) ?? "",
                    NoticeLateFeeDate: formatDate(item.NoticeLateFeeDate as string) ?? ""
                };
            });
    ...
    const handleFileUpload = (data: IConfirmDelinquenciesImportItems[]) => {
        if (data.length === 0) {
            setToggleSpinner(false);
            toast.error(
       ...
       const trimmedData = data.map(record => {
        const trimmedRecord: Partial<IConfirmDelinquenciesImportItems> = {};
        for (const key in record) {
            if (record.hasOwnProperty(key)) {
                const trimmedKey = key.trim();
                if (trimmedKey !== "") {
                    const value = record[key as keyof IConfirmDelinquenciesImportItems];
                    if (typeof value === 'string') {
                        trimmedRecord[trimmedKey as keyof IConfirmDelinquenciesImportItems] = value.trim() as any;
                    } else {
                        trimmedRecord[trimmedKey as keyof IConfirmDelinquenciesImportItems] = value?.toString() as any;
                    }
                }
            }
        }
        return trimmedRecord as IConfirmDelinquenciesImportItems;
    });
        const keys = Object.keys(trimmedData[0]);
        const headerMatches = keys.every((key) =>
            ConfirmDelinquenciesNoticesCSVHeader.includes(key)
        );

        if (headerMatches && ConfirmDelinquenciesNoticesCSVHeader.length === keys?.length) {
                            ...
                            <div className="text-center">
                                ...
                                <div className="mt-1.5 text-xs leading-5 text-[#2472db]">
                                    ...
                                    <p className="w-full text-xs mt-3 text-[#2472db]">
                                        <DownloadButton
                                            headers={ConfirmDelinquenciesNoticesCSVHeader}
                                            fileName={"notices"}
                        ...
                        <>
                            ...
                            <div className="preview-data">
                                ...
                                <Grid
                                    ...
                                    showInPopUp={true}
                                    cellRenderer={(
                                        data: IConfirmDelinquenciesImportItems,
                                        rowIndex: number,
                                        cellIndex: number
                                    ) => {
                                        ...
                                        const cellValue =
                                            data[columnName as keyof IConfirmDelinquenciesImportItems];
                                                ...
                                                >
                                                    ...
                                                    <div className="relative text-left max-w-[120px]">
                                                        ...
                                                        >
                                                            ...
                                                            <option value={typeof cellValue === 'string' || typeof cellValue === 'number' ? cellValue : undefined}>
                                                ...
                                                >
                                                    ...
                                                    <div className="relative text-left max-w-[120px]">
                                                        ...
                                                        >
                                                            ...
                                                            <option value={typeof cellValue === 'string' || typeof cellValue === 'number' ? cellValue : undefined}>
                                                                {typeof cellValue === 'string' || typeof cellValue === 'number' ? cellValue : 'Invalid value'}
                                                            </option>
                                                            {/* Populate other options with the values from StateCode array */}
                                                            {StateCode.map(
                                                                (state) =>
                              ...
                              <div key={index} className="text-red-500 whitespace-normal">
                                {error.errorMessage}
                                        ...
                                        if (columnName === "NoticeDeliveryDate" || columnName === "NoticeDefaultStartDate" || columnName === "NoticeDefaultEndDate" || columnName === "NoticeLastPaidDate" || columnName==="NoticeLateFeeDate") {
                                            return (
                                                <td
                                                    key={cellIndex}
                                                    className="px-1.5 py-2 md:py-2.5 font-normal text-[10.5px] text-gray-900 whitespace-nowrap"
                                        ...
                                        if (columnName=="NoticeLastPaidAmount" || columnName=="MonthlyRent" || columnName=="NoticeCurrentRentDue"|| columnName=="NoticePastRentDue" || columnName=="NoticeLateFees") {
                                            return (
                                               <td
                                                  key={cellIndex}
                                                  className="px-1.5 py-2 md:py-2.5 font-normal text-[10.5px] text-gray-900 whitespace-nowrap "
                                         ...
                                         if (columnName === "AttorneyBarNo" || columnName === "ClientReferenceId" || columnName === "NoticeServerID") {
                                            return (
                                                <td
                                                key={cellIndex}
                                                className={
                                            ...
                                            >
                                                <input
                                                    type={"text"}
                                                    value={(cellValue as any)}
                                                    className={`peer outline-none p-2 py-1 block border w-full rounded-md text-[10.5px] placeholder:text-transparent focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none h-[30px] 
                                                        }`}
                                                    onChange={(e) =>
                                                        handleInputChange?.(
                                                            columnName,
...
export default ConfirmDelinquenciesNotices_ImportCsv;
