...
import { formattedDate, mapDismissalReasonToEnum, toCssClassName } from "utils/helper";
import { UserRole } from "utils/enum";
import { useAuth } from "context/AuthContext";
import { useDissmissalsContext } from 'pages/dismissals/DismissalsContext';
...
const initialColumnMapping: IGridHeader[] = [
    ...
    {columnName:"currentBalance",label:"CurrentBalance"},
    { columnName: "companyName", label: "Company" },
 ];

const AddtoDismissals = (props: AddtoDismissalProps) => {
   ...
   useEffect(() => {
      ...
      const allCasesRecords =evictionAutomationDismissalApprovalsQueue.items.map((item: any) => ({
         ...item,
         reason: "Without Prejudice",
      }));

      let records: React.SetStateAction<IFileEvictionsItems[]> = [];

      const selectedIds = selectedEvictionDismissalApprovalId;
   ...
   }, [evictionAutomationDismissalApprovalsQueue.items, selectedEvictionDismissalApprovalId]);
   ...
   const fileDismissalsAsNonSigner = async () => {
      try {
         ...
         const response = await AllCasesService.nonSignFileDismissal(
            dismissalPayload
          );
      ...
      const renderers: Record<string, () => JSX.Element> = {
         ...
         reason: () => (
            <DropdownPresentation
               ...
               handleSelect={(event) => { handleDropdownChange(event, data.id); handleInputChange?.(propertyName, event.target.value, rowIndex); } }
               options={ReasonList}
            />
         ),
         document: () =>
               ...
               <div className="bg-white px-3.5 pb-3.5 pt-4 sm:p-5 sm:pb-3.5">
                  <div className="text-center pr-4 sm:text-left">
                     <h3
                        ...
                        id="modal-title"
                     >
                        Are you sure you like to confirm these cases for dismissal?
                     </h3>
                  </div>
               </div>
...
export default AddtoDismissals;
