...
import { ICRMTranctionCodeList, ICRMTransactionCodesItem, IEvictionAutomationIntegrationsGridItem, IEvictionAutomationPropexoGridItem, ITranctionCodeList, ITransactionCodesItem } from "interfaces/eviction-automation.interface";
...
type TransactionCodesFormPopupProps = {
    ...
    isEditMode: boolean;
    initialValues: ICRMTransactionCodesItem;
    onSubmit: (formValues: ICRMTransactionCodesItem) => void;
};

const CRMImporterTransactionCodesFormPopup: React.FC<TransactionCodesFormPopupProps> = ({
    showPopup,
    closePopup,
    isEditMode,
    initialValues,
    ...
    const [transactionCodeList, setTransactionCodeList] = useState<ICRMTranctionCodeList[]>([]);
    ...
    const [pmsId, setPMSId] = useState<string>('');
    const { userRole } = useAuth();

    const [selectedValues, setSelectedValues] = useState([
        { id: "", value: "", disabled: false },
    ]);
    const initialFormValues: ICRMTransactionCodesItem = {
        id: initialValues.id ?? "",
        ...
        crmTransactionCodes: Array.isArray(initialValues?.crmTransactionCodes) && initialValues?.crmTransactionCodes.length
            ? initialValues.crmTransactionCodes
            : [
                {
                    transactionCodeId: "",
                    transactionCodeName: "",
   ...
   CompanyPropertyTransactionCodeId: "",
    ...
    useEffect(() => {
        if (initialFormValues.propertyId != null && initialFormValues.integrationId != null && initialFormValues.propertyId !== "" && initialFormValues.integrationId !== "") {
            ...
            fetchPropertiesBasedOnPMS(initialFormValues.integrationId ?? "");
    ...
    const getTransactionCodesById = async (propertyId: string, integrationId: string) => {
        try {
            ...
            const response = await EvictionAutomationService.getPropertyTransactionCodeById(propertyId, integrationId);
    ...
    const handleDropdownChange = (
        ...
        formik: FormikProps<ICRMTransactionCodesItem>
    ) => {
        const isValueAlreadyAdded = formik.values.crmTransactionCodes.some(
            (code) => code.id === selectedValue
        );
        ...
        const filteredTransactionCode = transactionCodeList.find((x) => x.id === selectedValue);
        if (filteredTransactionCode) {
            // Update the corresponding transaction code in the formik values
            let updatedTransactionCodes = [...formik.values.crmTransactionCodes];
            updatedTransactionCodes[index] = {
                ...updatedTransactionCodes[index],
                id: selectedValue,
                transactionCodeName: filteredTransactionCode.codeName,
                transactionCodeDescription: filteredTransactionCode.codeDescription,
            };

            // Update Formik's field value for the entire crmTransactionCodes array
            formik.setFieldValue("crmTransactionCodes", updatedTransactionCodes);
    ...
    const handleSelectIntegrationChange = async (event: ChangeEvent<HTMLSelectElement>) => {
        ...
        setPMSId(integrationId);
        try {
            setShowSpinner(true);
            //fetchOwnerListByIntegrationId(integrationId);
            fetchPropertiesBasedOnPMS(integrationId);
    ...
    const fetchPropertiesBasedOnPMS = async (integrationId: string) => {
        try {
            ...
            const apiResponse = await EvictionAutomationService.getPropertyBasedOnPMS(integrationId);
    ...
    const renderTransactionCodeOptions = (options: ICRMTranctionCodeList[], index: number) => {
        return (
            <>
                ...
                {options.map((option) => (
                    <option key={option.id} value={option.id}
                        title={`${option.codeName ? option.codeName + (option.codeDescription ? ' - ' + option.codeDescription : '') : ''}`}>
                        {option.codeName}
                    </option>
                ))}
            </>
    ...
    const handlePlusClick = (formik: FormikProps<ICRMTransactionCodesItem>) => {
        ...
        formik.setFieldValue("crmTransactionCodes", [...formik.values.crmTransactionCodes, newTransaction]);
    ...
    const handleToggleTransactionCodes = (formik: FormikProps<ICRMTransactionCodesItem>) => {
        setAreAllTransactionCodesAdded((prevState) => {
            const currentSelectedTransactionCodes = formik.values.crmTransactionCodes.filter((code) => code.id !== '').map(
                (code) => code.id
            );
            ...
            if (prevState) {
                ...
                const remainingTransactionCodes = formik.values.crmTransactionCodes.filter(
                    (transactionCode) => !addedTransactionCodes.includes(transactionCode.transactionCodeId)
                );

                formik.setFieldValue("crmTransactionCodes", remainingTransactionCodes);
                ...
                const missingTransactionCodes = transactionCodeList
                    .filter((transactionCode) => !selectedTransactionIds.includes(transactionCode.id))
                    .filter((transactionCode) => transactionCode.id !== "default"); // Exclude default or placeholder codes
                ...
                if (missingTransactionCodes.length > 0) {
                    const newTransactionCodes = missingTransactionCodes.map((transactionCode) => ({
                        ...
                        transactionCodeId: transactionCode.id,
                        transactionCodeName: transactionCode.codeName,
                        transactionCodeDescription: transactionCode.codeDescription,
                        isRent: false,
                        isSubsidy: false,
                    ...
                    formik.setFieldValue("crmTransactionCodes", [
                        ...formik.values.crmTransactionCodes,
                        ...newTransactionCodes,
                    ]);
                    ...
                    setAddedTransactionCodes((prevCodes) => [
                        ...prevCodes,
                        ...missingTransactionCodes.map((transactionCode) => transactionCode.id),
                    ]);
    ...
    const areAllTransactionCodesSelected = (formik: FormikProps<ICRMTransactionCodesItem>): boolean => {
        const selectedTransactionIds = formik.values.crmTransactionCodes.map(
            (code) => code.id
        );

        // Check if every transaction code in the list is already selected
        return transactionCodeList
            .filter((code) => code.id !== "default") // Exclude default or placeholder codes
            .every((code) => selectedTransactionIds.includes(code.id));
    };
    ...
    const handleCrossClick = (
        ...
        if (formik.values.crmTransactionCodes && formik.values.crmTransactionCodes.length >= 1) {
            // Use filter to create a new array without the element at the specified _index
            let filteredRecords = formik.values.crmTransactionCodes.filter(
                (_, index) => index !== _index
            );
            // Update the form data model using Formik's setFieldValue
            formik.setFieldValue("crmTransactionCodes", filteredRecords);
        }
    };
                                            ...
                                            <div className="relative text-left">
                                                {
                                                    <FormikControl
                                                        ...
                                                        onChange={(e: React.ChangeEvent<HTMLSelectElement>) => {
                                                            const property = propertyList.find(x => x.id === e.target.value);
                                                            if (property) {
                                                                formik.setFieldValue("propertyId", property.id);
                                                                formik.setFieldValue("propertyName", property.value);
                                                                setIsPropertySelected(true);
                                                                  getTransactionCodesById(property.id?.toString() ?? "", pmsId ?? "");
                                                            }
                                                            else {
                                        ...
                                        {isPropertySelected &&
                                            <div className="court_payments">
                                                ...
                                                <div className="overflow-auto" style={{ maxHeight: '18rem' }}>
                                                    {
                                                        formik.values.crmTransactionCodes &&
                                                        formik.values.crmTransactionCodes.map((value, index) => {
                                                            if (formik.values.crmTransactionCodes.length - 1 === transactionCodeList.length) {
                                                                formik.values.crmTransactionCodes = formik.values.crmTransactionCodes.filter(x => x.id !== '')
                                                            }
                                                            const selectedTransactionIds = formik.values.crmTransactionCodes
                                                                .slice(0, index)
                                                                    ...
                                                                    <div className="flex flex-col sm:flex-row gap-2 md:w-[42%] lg:w-[44%] pt-1 md:pt-0">
                                                                        ...
                                                                        <div className="sm:w-[50%]">
                                                                            ...
                                                                            <input
                                                                                ...
                                                                                onChange={(e) => {
                                                                                    formik.setFieldValue(`crmTransactionCodes[${index}].transactionCodeShortDescription`, e.target.value);
                                                                                }}
                                                                        ...
                                                                        <div className="relative text-left">
                                                                            ...
                                                                            <div className="flex items-center flex-row-reverse justify-end gap-1.5" key={`${index}_isRent`}>
                                                                                <FormikControl
                                                                                    ...
                                                                                    type="checkbox"
                                                                                    label={"Is Rent"}
                                                                                    name={`crmTransactionCodes[${index}].isRent`}
                                                                                    checked={value.isRent}
                                                                                    onChange={(isChecked: boolean) => {
                                                                                        handleCheckboxChange(
                                                                                            `crmTransactionCodes[${index}].isRent`,
                                                                                            isChecked,
                                                                                            formik
                                                                                        );
                                                                                    }}
                                                                        ...
                                                                        <div className="relative text-left">
                                                                            ...
                                                                            <div className="flex items-center flex-row-reverse justify-end gap-1.5" key={`${index}_isSubsidy`}>
                                                                                <FormikControl
                                                                                    ...
                                                                                    type="checkbox"
                                                                                    label={"Is Subsidy"}
                                                                                    name={`crmTransactionCodes[${index}].isSubsidy`}
                                                                                    checked={value.isSubsidy}
                                                                                    onChange={(isChecked: boolean) => {
                                                                                        handleCheckboxChange(
                                                                                            `crmTransactionCodes[${index}].isSubsidy`,
                                                                                            isChecked,
                                                                                            formik
                                                                                        );
                                                                                    }}
...
export default CRMImporterTransactionCodesFormPopup;
