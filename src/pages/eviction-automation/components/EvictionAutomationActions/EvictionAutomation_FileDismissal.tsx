...
const AllCases_AddtoDismissals = (props: AddtoDismissalProps) => {
   ...
   useEffect(() => {
      ...
      const allCasesRecords =props.activeTab==="Confirm Dismissals"? evictionAutomationDismissalApprovalsQueue.items.map((item: any) => ({
         ...item,
         reason: "Without Prejudice",
      })):evictionAutomationDismissalApprovedQueue.items.map((item: any) => ({
        ...item,
        reason: "Without Prejudice",
     }));
      ...
      const selectedIds = props.activeTab==="Confirm Dismissals"
         ? selectedEvictionDismissalApprovalId
         : selectedEvictionDismissalApprovedId;
   ...
   }, [evictionAutomationDismissalApprovalsQueue.items, selectedEvictionDismissalApprovalId,evictionAutomationDismissalApprovedQueue.items,selectedEvictionDismissalApprovedId]);
   ...
   const fileDismissalsAsNonSigner = async () => {
      try {
         setShowConfirm(true);
        //  const response = await AllCasesService.nonSignFileDismissal(selectedReason);
