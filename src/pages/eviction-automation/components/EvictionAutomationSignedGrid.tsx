...
const EvictionAutomationSignedGrid: React.FC = () => {
    const {
       showSpinner,
    ...
    const [canPaginateBack, setCanPaginateBack] = useState<boolean>(evictionAutomationSignedQueue.currentPage > 1);
    const [canPaginateFront, setCanPaginateFront] = useState<boolean>(evictionAutomationSignedQueue.totalPages > 1);
    const [selectedRows, setSelectedRows] = useState<Array<boolean>>(
       Array(evictionAutomationSignedQueue.items.length).fill(false)
    );
    ...
    useEffect(() => {
      getEvictionAutomationSignedQueue(1, 100,);
      ...
      //  );
       ...
       setCanPaginateBack(evictionAutomationSignedQueue.currentPage > 1);
       setCanPaginateFront(evictionAutomationSignedQueue.totalPages > 1);
    ...
    const handleCheckBoxChange = (index: number, id: string, checked: boolean) => {
       ...
       if (shiftKeyPressed && lastClickedRowIndex !== -1 && evictionAutomationSignedQueue.items) {
          ...
          const selectedIds = (evictionAutomationSignedQueue.items || [])
             .filter((_, rowIndex) => selectedRows[rowIndex])
             .map((item) => item.id)
             .filter((id): id is string => typeof id === "string");
 
             evictionAutomationSignedQueue.items.filter((_, rowIndex) => selectedRows[rowIndex]).map((item) => {
         ...
         // setSelectedEvictionApprovedId(prevIds => [...new Set([...prevIds, ...selectedIds])]);
          ...
          if (evictionAutomationSignedQueue.items.length === updatedSelectedRows.filter(item => item).length) {
             setSelectAll(true);
          } else {
             setSelectAll(false);
          }
 
          var selectedIds = evictionAutomationSignedQueue.items.filter(item => item.id == id).map((item) => item.id)
             .filter((id): id is string => typeof id === "string");
    ...
    const handleCellRendered = (cellIndex: number, data: IFileEvictionsItems, rowIndex: number) => {
       ...
       const renderers: Record<string, () => JSX.Element> = {  
         ...
         //  ),
          ...
          companyName: () => <HighlightedText text={cellValue ?? ""} query={evictionAutomationSignedQueue.searchParam ?? ''} />,
          andAllOtherTenants: () => <HighlightedText text={cellValue ?? ""} query={evictionAutomationSignedQueue.searchParam ?? ''} />,
          attorneyBarNo: () => <HighlightedText text={cellValue ?? ""} query={evictionAutomationSignedQueue.searchParam ?? ''} />,
          attorneyEmail: () => <HighlightedText text={cellValue ?? ""} query={evictionAutomationSignedQueue.searchParam ?? ''} />,
          attorneyName: () => <HighlightedText text={cellValue ?? ""} query={evictionAutomationSignedQueue.searchParam ?? ''} />,
          county: () => <HighlightedText text={cellValue ?? ""} query={evictionAutomationSignedQueue.searchParam ?? ''} />,
          filerEmail: () => <HighlightedText text={cellValue ?? ""} query={evictionAutomationSignedQueue.searchParam ?? ''} />,
          evictionAffiantIs: () => <HighlightedText text={cellValue ?? ""} query={evictionAutomationSignedQueue.searchParam ?? ''} />,
          filerBusinessName: () => <HighlightedText text={cellValue ?? ""} query={evictionAutomationSignedQueue.searchParam ?? ''} />,
          ownerName: () => <HighlightedText text={cellValue ?? ""} query={evictionAutomationSignedQueue.searchParam ?? ''} />,
          propertyAddress: () => <HighlightedText text={cellValue ?? ""} query={evictionAutomationSignedQueue.searchParam ?? ''} />,
          propertyCity: () => <HighlightedText text={cellValue ?? ""} query={evictionAutomationSignedQueue.searchParam ?? ''} />,
          propertyEmail: () => <HighlightedText text={cellValue ?? ""} query={evictionAutomationSignedQueue.searchParam ?? ''} />,
          propertyName: () => <HighlightedText text={cellValue ?? ""} query={evictionAutomationSignedQueue.searchParam ?? ''} />,
          propertyPhone: () => <HighlightedText text={cellValue ?? ""} query={evictionAutomationSignedQueue.searchParam ?? ''} />,
          propertyState: () => <HighlightedText text={cellValue ?? ""} query={evictionAutomationSignedQueue.searchParam ?? ''} />,
          propertyZip: () => <HighlightedText text={cellValue ?? ""} query={evictionAutomationSignedQueue.searchParam ?? ''} />,
       };
    ...
    const handleFrontButton = () => {
       if (evictionAutomationSignedQueue.currentPage < evictionAutomationSignedQueue.totalPages) {
          const updatedCurrentPage = evictionAutomationSignedQueue.currentPage + 1;
          // Update current page and enable/disable 'Back' button
          setCanPaginateBack(updatedCurrentPage > 1);
          // back button get late notices
          getEvictionAutomationSignedQueue(updatedCurrentPage,
            evictionAutomationSignedQueue.pageSize);
       }
    };
 
    const handleBackButton = () => {
       if (
         evictionAutomationSignedQueue.currentPage > 1 &&
         evictionAutomationSignedQueue.currentPage <= evictionAutomationSignedQueue.totalPages
       ) {
          const updatedCurrentPage = evictionAutomationSignedQueue.currentPage - 1;
          // Update current page and enable/disable 'Back' button
          setCanPaginateBack(evictionAutomationSignedQueue.currentPage > 1);
    ...
    const handleSelectAllChange = (checked: boolean) => {
       ...
       const allIds: string[] = evictionAutomationSignedQueue.items
   .filter((item: IFileEvictionsItems) => item.isSigned === false)
   .map((item) => item.id)
   .filter((id): id is string => id !== undefined);
       if (checked) {
         ...
         evictionAutomationSignedQueue.items.forEach((item) => {
    ...
    return (
       <div>
          ...
          <Grid
             ...
             rows={evictionAutomationSignedQueue.items}
             handleSelectAllChange={handleSelectAllChange}
             //selectAll={checkIfAllIdsExist(evictionAutomationApprovedQueue.items.filter((item: IFileEvictionsItems) => item.isSigned === false), selectedEvictionApprovedId)}
             cellRenderer={(data: IFileEvictionsItems, rowIndex: number, cellIndex: number) =>
                handleCellRendered(cellIndex, data, rowIndex)
             }
          />
          <Pagination
             numberOfItemsPerPage={evictionAutomationSignedQueue.pageSize}
             currentPage={evictionAutomationSignedQueue.currentPage}
             totalPages={evictionAutomationSignedQueue.totalPages}
             totalRecords={evictionAutomationSignedQueue.totalCount}
 ...
 export default EvictionAutomationSignedGrid;
