...
const initialColumnMapping: IGridHeader[] = [
   ...
   { columnName: "applicantDate", label: "WritApplicantDate" },
   ...
   { columnName: "filedDate", label: "WritFiledDate" },
   { columnName: "courtTransAmount", label: "WritCourtTransAmount", className:'text-right' },
   { columnName: "c2CFilingFee", label: "C2CWritFee", className:'text-right' },
   { columnName: "paymentAmount", label: "WritPaymentAmount", className:'text-right' },
   { columnName: "payPalFee", label: "WritPayPalFee", className:'text-right' },
   { columnName: "payPalManual", label: "WritPayPalManual", className:'text-right' },
   { columnName: "filingCount", label: "FilingCount", className:'text-right' },
   { columnName: "paymentMethod", label: "eFilePaymentMethod" },
   { columnName: "invoiceDate", label: "WritInvoiceDate" },
   { columnName: "invoiceNumber", label: "WritInvoice#" },
   { columnName: "datePaid", label: "WritDatePaid" },
   { columnName: "checkNumber", label: "WritCheck#" },
...
const FilingWritGrid = (props: FilingTransactionProps) => {
   ...
   const [canPaginateBack, setCanPaginateBack] = useState<boolean>(writsFilingTransactions.currentPage > 1);
   const [canPaginateFront, setCanPaginateFront] = useState<boolean>(writsFilingTransactions.totalPages > 1);
   ...
   const [selectedRows, setSelectedRows] = useState<Array<boolean>>(Array(writsFilingTransactions.items?.length).fill(false));
   ...
   useEffect(() => {
      const filingTransactionsRecords: IFilingTransactionItem[] = writsFilingTransactions.items.map((item: any) => {
         return {
      ...
      const updatedSelectedRows = (writsFilingTransactions.items || []).map((item: any) =>
      ...
      setCanPaginateBack(writsFilingTransactions.currentPage > 1);
      setCanPaginateFront(writsFilingTransactions.totalPages > 1);
   ...
   }, [writsFilingTransactions, selectedFilingTransactionId]);
   ...
   const handleCheckBoxChange = (index: number, id: string, checked: boolean) => {
      ...
      if (shiftKeyPressed && lastClickedRowIndex !== -1 && filingTransactionsRecords) {
         ...
         if (!checked) {
            ...
            setWritBulkRecords(prevItems => prevItems.filter(item => item.id !== id));
   ...
   const handleSelectAllChange = (checked: boolean) => {
      ...
      if (checked) {
         ...
         filingTransactionsRecords.forEach((item) => {
            setWritBulkRecords(prevItems => prevItems.filter(record => record.id !== item.id));
   ...
   const checkIfAllIdsExist = (
      filingWritRecords: IFilingTransactionItem[],
      selectedFilingWritId: string[]
   ): boolean | undefined => {
      if (filingWritRecords.length === 0) {
          return false;
      }
      return filingWritRecords.every(record =>
         selectedFilingWritId.includes(record.id as string)
      );
   };
   ...
   // );
   const formattedTenantFullName = (tenant: ITenant | null | undefined) => (
      <HighlightedText text={`${tenant?.firstName ?? ''} ${tenant?.middleName ?? ""} ${tenant?.lastName ?? ''}`} query={writsFilingTransactions.searchParam ?? ''} />
   );
   const formattedCell = (value: any) => (
      <span>{value !== null ? value : ""}</span>
   );
   ...
   const renderHighlightedCell = (value: any) => (
      <HighlightedText text={value as string ?? ''} query={writsFilingTransactions.searchParam ?? ''} />
   );
   ...
   const handleFrontButton = () => {
      if (writsFilingTransactions.currentPage < writsFilingTransactions.totalPages) {
         const updatedCurrentPage = writsFilingTransactions.currentPage + 1;
         setCanPaginateBack(updatedCurrentPage > 1);
         getFilingTransactions(
            updatedCurrentPage,
            writsFilingTransactions.pageSize,
            writsFilingTransactions.searchParam,
            "Writ",
            companyId,
            writsFilingTransactions.fromDate,
            writsFilingTransactions.toDate,
            writsFilingTransactions.datePaidFromDate,
            writsFilingTransactions.datePaidToDate,
            writsFilingTransactions.blankOption
            ,writsFilingTransactions.nonBlankOption
            ,writsFilingTransactions.county
         );
      }
   };
   ...
   const handleBackButton = () => {
      if (
         writsFilingTransactions.currentPage > 1 &&
        writsFilingTransactions.currentPage <= writsFilingTransactions.totalPages
      ) {
         const updatedCurrentPage = writsFilingTransactions.currentPage - 1;
         setCanPaginateBack(writsFilingTransactions.currentPage > 1);
         getFilingTransactions(
   ...
   //       getFilingTransactions(1,100,filingTransactions.searchParam,OperationTypeEnum.Writ,companyId);
   //    }
   ...
   return (
      <div className="pt-1.5 lg:pt-2 accounting_grid">
         ...
         <div className="mb-2 text-sm text-gray-600">
            {selectedFilingTransactionId.length} of {writsFilingTransactions.totalCount} records selected
         </div>
            ...
            <div className="relative -mr-0.5">
               ...
               <>
                  <Grid
                     ...
                     rows={writsFilingTransactions.totalCount ? writsFilingTransactions.items : []}
                  ...
                  <Pagination
                     numberOfItemsPerPage={writsFilingTransactions.pageSize}
                     currentPage={writsFilingTransactions.currentPage}
                     totalPages={writsFilingTransactions.totalPages}
                     totalRecords={writsFilingTransactions.totalCount}
