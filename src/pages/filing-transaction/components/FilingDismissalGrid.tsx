...
const initialColumnMapping: IGridHeader[] = [
   ...
   { columnName: "applicantDate", label: "DismissalApplicantDate" },
   ...
   { columnName: "filedDate", label: "DismissalFiledDate" },
   { columnName: "courtTransAmount", label: "DismissalCourtTransAmount", className:'text-right' },
   { columnName: "c2CFilingFee", label: "C2CDismissalFee", className:'text-right' },
   { columnName: "paymentAmount", label: "DismissalPaymentAmount", className:'text-right' },
   { columnName: "payPalFee", label: "DismissalPayPalFee", className:'text-right' },
   { columnName: "payPalManual", label: "DismissalPayPalManual", className:'text-right' },
   { columnName: "filingCount", label: "FilingCount", className:'text-right' },
   { columnName: "paymentMethod", label: "eFilePaymentMethod" },
   { columnName: "invoiceDate", label: "DismissalInvoiceDate" },
   { columnName: "invoiceNumber", label: "DismissalInvoice#" },
   { columnName: "datePaid", label: "DismissalDatePaid" },
   { columnName: "checkNumber", label: "DismissalCheck#" },
...
const FilingDismissalGrid = (props: FilingTransactionProps) => {
   ...
   const [canPaginateBack, setCanPaginateBack] = useState<boolean>(dismissalsFilingTransactions.currentPage > 1);
   const [canPaginateFront, setCanPaginateFront] = useState<boolean>(dismissalsFilingTransactions.totalPages > 1);
   ...
   const [selectedRows, setSelectedRows] = useState<Array<boolean>>(Array(dismissalsFilingTransactions.items?.length).fill(false));
   ...
   useEffect(() => {
      const filingTransactionsRecords: IFilingTransactionItem[] = dismissalsFilingTransactions.items.map((item: any) => {
         return {
      ...
      const updatedSelectedRows = (dismissalsFilingTransactions.items || []).map((item: any) =>
      ...
      setCanPaginateBack(dismissalsFilingTransactions.currentPage > 1);
      setCanPaginateFront(dismissalsFilingTransactions.totalPages > 1);
   ...
   }, [dismissalsFilingTransactions, selectedFilingTransactionId]);
   ...
   const settingData = async (record: IFilingTransactionItem) => {
      ...
      setDismissalBulkRecords(prevItems => {
   ...
   const handleCheckBoxChange = (index: number, id: string, checked: boolean) => {
      ...
      if (shiftKeyPressed && lastClickedRowIndex !== -1 && filingTransactionsRecords) {
         ...
         if (!checked) {
            ...
            setDismissalBulkRecords(prevItems => prevItems.filter(item => item.id !== id));
   ...
   const handleSelectAllChange = (checked: boolean) => {
      ...
      if (checked) {
         ...
         filingTransactionsRecords.forEach((item) => {
            setDismissalBulkRecords(prevItems => prevItems.filter(record => record.id !== item.id));
   ...
   // };
   const checkIfAllIdsExist = (
      filingDismissalRecords: IFilingTransactionItem[],
      selectedFilingDismissalId: string[]
   ): boolean | undefined => {
      if (filingDismissalRecords.length === 0) {
          return false;
      }
      return filingDismissalRecords.every(record =>
         selectedFilingDismissalId.includes(record.id as string)
      );
   };
   ...
   // );
   const formattedTenantFullName = (tenant: ITenant | null | undefined) => (
      <HighlightedText text={`${tenant?.firstName ?? ''} ${tenant?.middleName ?? ""} ${tenant?.lastName ?? ''}`} query={dismissalsFilingTransactions.searchParam ?? ''} />
   );
   ...
   const renderHighlightedCell = (value: any) => (
      <HighlightedText text={value as string ?? ''} query={dismissalsFilingTransactions.searchParam ?? ''} />
   );
   ...
   const handleFrontButton = () => {
      if (dismissalsFilingTransactions.currentPage < dismissalsFilingTransactions.totalPages) {
         const updatedCurrentPage = dismissalsFilingTransactions.currentPage + 1;
         setCanPaginateBack(updatedCurrentPage > 1);
         getFilingTransactions(
            updatedCurrentPage,
            dismissalsFilingTransactions.pageSize,
            dismissalsFilingTransactions.searchParam,
            "Dismissal",
            companyId,
            dismissalsFilingTransactions.fromDate,
            dismissalsFilingTransactions.toDate,
            dismissalsFilingTransactions.datePaidFromDate,
            dismissalsFilingTransactions.datePaidToDate,
            dismissalsFilingTransactions.blankOption
            ,dismissalsFilingTransactions.nonBlankOption
            ,dismissalsFilingTransactions.county
         );
      }
   };
   ...
   const handleBackButton = () => {
      if (
         dismissalsFilingTransactions.currentPage > 1 &&
         dismissalsFilingTransactions.currentPage <= dismissalsFilingTransactions.totalPages
      ) {
         const updatedCurrentPage = dismissalsFilingTransactions.currentPage - 1;
         setCanPaginateBack(dismissalsFilingTransactions.currentPage > 1);
         getFilingTransactions(
   ...
   //       getFilingTransactions(1,100,filingTransactions.searchParam,OperationTypeEnum.Dismissal,companyId);
   //    }
   ...
   return (
      <div className="pt-1.5 lg:pt-2 accounting_grid">
         ...
         <div className="mb-2 text-sm text-gray-600">
            {selectedFilingTransactionId.length} of {dismissalsFilingTransactions.totalCount} records selected
         </div>
            ...
            <div className="relative -mr-0.5">
               ...
               <>
                  <Grid
                     ...
                     rows={dismissalsFilingTransactions.totalCount ? dismissalsFilingTransactions.items : []}
                  ...
                  <Pagination
                     numberOfItemsPerPage={dismissalsFilingTransactions.pageSize}
                     currentPage={dismissalsFilingTransactions.currentPage}
                     totalPages={dismissalsFilingTransactions.totalPages}
                     totalRecords={dismissalsFilingTransactions.totalCount}
...
export default FilingDismissalGrid;