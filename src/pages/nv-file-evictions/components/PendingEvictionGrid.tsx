...
const PendingEvictionGrid = (props: PendingEvictionGridProps) => {
    const { 
        pendingEvictions, 
        getNVEvictions, 
        showSpinner,
 ...
 useEffect(() => {   
    const lateNoticesRecords = pendingEvictions.items.map((item: any) => {
      return {
    ...
    setCanPaginateBack(pendingEvictions.currentPage > 1);
    setCanPaginateFront(pendingEvictions.totalPages > 1);
    setSelectedRows(Array(pendingEvictions.items?.length).fill(false));
    setSelectAll(false);
...
console.log(selectedNVEvictionId);
  ...
  const handleCellRendered = (
    ...
    const renderers: Record<string, () => JSX.Element> = {
      ...
      county: () => <HighlightedText text={cellValue ?? ""} query={pendingEvictions.searchParam ?? ''} />,
      isChecked: () => (
        <GridCheckbox
...
const formattedCurrencyCell = (value: any) => (
  ...
  const formattedAddressCell = (value: ILateNoticesItems) => (
    <span>
      <HighlightedText text={value !== null ? `${value.address ?? ''} ${value.unit ?? ''} ${value.city ?? ''} ${value.zip ?? ''}` : ''}
        query={pendingEvictions.searchParam ?? ''} /> </span>
  );
    ...
    <span>
      <HighlightedText text=
        {tenantNames && tenantNames.length > 0
          ...
          : ""} query={pendingEvictions.searchParam ?? ''} />
    </span>
  );
  const formattedPropertyCell = (value: any) => (
    <span><HighlightedText text={value !== null ? value : ""} query={pendingEvictions.searchParam ?? ''} /></span>
  );
  ...
  const handleFrontButton = () => {
    if (pendingEvictions.currentPage < pendingEvictions.totalPages) {
      const updatedCurrentPage = pendingEvictions.currentPage + 1;
      // Update current page and enable/disable 'Back' button
      setCanPaginateBack(updatedCurrentPage > 1);
      // back button get late notices
      getNVEvictions(
        updatedCurrentPage,
        pendingEvictions.pageSize,
        pendingEvictions.isConfirmed??false,
        pendingEvictions.searchParam
      );
    }
  };
  const handleBackButton = () => {
    if (
        pendingEvictions.currentPage > 1 &&
        pendingEvictions.currentPage <= pendingEvictions.totalPages
    ) {
      const updatedCurrentPage = pendingEvictions.currentPage - 1;
      // Update current page and enable/disable 'Back' button
      setCanPaginateBack(pendingEvictions.currentPage > 1);
   ...
   return (<div className="relative -mr-0.5">
    <div className="mb-2 text-sm text-gray-600">
            {selectedNVEvictionId.length} of {pendingEvictions.totalCount} records selected
         </div>
    ...
    {showSpinner === true ? (
      ...
      <>
        ...
        <Pagination
          numberOfItemsPerPage={pendingEvictions.pageSize}
          currentPage={pendingEvictions.currentPage}
          totalPages={pendingEvictions.totalPages}
          totalRecords={pendingEvictions.totalCount}
...
export default PendingEvictionGrid;