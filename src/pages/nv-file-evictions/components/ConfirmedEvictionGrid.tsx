...
type ConfirmedEvictionGridProps = {};
...
const ConfirmEvictionGrid = (props: ConfirmedEvictionGridProps) => {
    const { 
        confirmedEvictions, 
        getNVEvictions, 
        showSpinner,
 ...
 useEffect(() => {    
    const lateNoticesRecords = confirmedEvictions.items.map((item: any) => {
      return {
    ...
    setCanPaginateBack(confirmedEvictions.currentPage > 1);
    setCanPaginateFront(confirmedEvictions.totalPages > 1);
    setSelectedRows(Array(confirmedEvictions.items?.length).fill(false));
    setSelectAll(false);
...
const handleSelectAllChange = (checked: boolean) => {
    ...
    if (checked) {
      ...
      setSelectedNVEvictionId(prevIds => [...new Set([...prevIds, ...allIds])]);    
    } else {
        confirmedEvictionRecords.forEach((item) => {
       setBulkRecords(prevItems => prevItems.filter(record => record.id !== item.id));
       setSelectedNVEvictionId(prevIds => prevIds.filter(id => id !== item.id));
  });
    }
  ...
  const handleCheckBoxChange = (index: number, id: string, checked: boolean) => {
    if (shiftKeyPressed && lastClickedRowIndex !== -1 && confirmedEvictionRecords) {
       ...
       setSelectedNVEvictionId(prevIds => [...new Set([...prevIds, ...selectedIds])]);
    } else {
       ...
       if (!checked) {
          ...
          setSelectedNVEvictionId(prevIds => prevIds.filter(item => item !== id));
       } else {
  ...
  const handleCellRendered = (
    ...
    const renderers: Record<string, () => JSX.Element> = {
      ...
      county: () => <HighlightedText text={cellValue ?? ""} query={confirmedEvictions.searchParam ?? ''} />,
      isChecked: () => (
        <GridCheckbox
          //checked={selectedRows[rowIndex]}
          checked={selectedNVEvictionId.includes(data.id as string)}
...
const formattedCurrencyCell = (value: any) => (
  ...
  const formattedAddressCell = (value: ILateNoticesItems) => (
    <span>
      <HighlightedText text={value !== null ? `${value.address ?? ''} ${value.unit ?? ''} ${value.city ?? ''} ${value.zip ?? ''}` : ''}
        query={confirmedEvictions.searchParam ?? ''} /> </span>
  );
    ...
    <span>
      <HighlightedText text=
        {tenantNames && tenantNames.length > 0
          ...
          : ""} query={confirmedEvictions.searchParam ?? ''} />
    </span>
  );
  const formattedPropertyCell = (value: any) => (
    <span><HighlightedText text={value !== null ? value : ""} query={confirmedEvictions.searchParam ?? ''} /></span>
  );
  ...
  const handleFrontButton = () => {
    if (confirmedEvictions.currentPage < confirmedEvictions.totalPages) {
      const updatedCurrentPage = confirmedEvictions.currentPage + 1;
      // Update current page and enable/disable 'Back' button
      setCanPaginateBack(updatedCurrentPage > 1);
      // back button get late notices
      getNVEvictions(
        updatedCurrentPage,
        confirmedEvictions.pageSize,
        confirmedEvictions.isConfirmed??true,
        confirmedEvictions.searchParam
      );
    }
  };
  const handleBackButton = () => {
    if (
      confirmedEvictions.currentPage > 1 &&
      confirmedEvictions.currentPage <= confirmedEvictions.totalPages
    ) {
      const updatedCurrentPage = confirmedEvictions.currentPage - 1;
      // Update current page and enable/disable 'Back' button
      setCanPaginateBack(confirmedEvictions.currentPage > 1);
   ...
   return (<div className="relative -mr-0.5">
    <div className="mb-2 text-sm text-gray-600">
            {selectedNVEvictionId.length} of {confirmedEvictions.totalCount} records selected
         </div>
    ...
    {showSpinner === true ? (
      ...
      <>
        <Grid
         ...
         // selectAll={selectAll}
          selectAll={checkIfAllIdsExist(confirmedEvictionRecords,selectedNVEvictionId)}
          cellRenderer={(
            data: ILateNoticesItems,
            rowIndex: number,
            cellIndex: number
        ...
        <Pagination
          numberOfItemsPerPage={confirmedEvictions.pageSize}
          currentPage={confirmedEvictions.currentPage}
          totalPages={confirmedEvictions.totalPages}
          totalRecords={confirmedEvictions.totalCount}
...
export default ConfirmEvictionGrid;